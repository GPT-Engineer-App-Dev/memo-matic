...
const supabase = createClient('https://jjfebbwwtcxyhvnkuyrh.supabase.co', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImpqZmViYnd3dGN4eWh2bmt1eXJoIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MTY0NTgyMzMsImV4cCI6MjAzMjAzNDIzM30.46syqx3sHX-PQMribS6Vt0RLLUY7w295JHO61yZ-fec');
...
const CreateNotePage = () => {
  ...
  const createNote = async () => {
    let imageUrl = null;
    if (image) {
      const { data, error } = await supabase.storage.from('notes').upload(`public/${image.name}`, image);
      if (error) {
